 ...
 *
 * Returns 0 for weekends, holidays, vacation days, and any weekday that has
 * an explicit zero in weekdayHours.  Otherwise returns the per-weekday
 * override (when weekdayHours is provided) or hoursPerDay.
 *
 ...
 * delegate here so that adding a new exclusion (e.g. comp days) only
 * requires one change.
 *
...
export function calcBookingHours(
  ...
  while (d <= e) {
    ...
    totalHours += calcDayHours(dow, dateStr, hoursPerDay, weekdayHours, holidaySet, vacationDateSet);
